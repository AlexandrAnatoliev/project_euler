...
* `libsieve_din.so` - объектный файл динамической библиотеки;
...
* `scripts/`	- скрипты для автоматизации процесса разработки и отладки библиотеки:
    ...
    - `create_dinamic_lib`  - скрипт для создания динамической библиотеки;
    - `file_din_lib_run`    - скрипт для компиляции файла с динамической библиотекой и его запуска;
    - `run_tests_din`       - перекомпиляция тестовых файлов динамической библиотеки и их запуск. 
...
`./create_dinamic_lib sieve`

Далее он самостоятельно создает библиотеку `libsieve_din.so` и удаляет временные файлы.

### file_din_lib_run
Скрипт для компиляции файла с динамической библиотекой и его запуска.

Как аргументы скрипт принимает название компилируемого файла и библиотеки. Название файлов писать без расширения и префикса `lib`. Например, `libsieve_din.so` сокращается до`sieve_din`, а `test.c` - до `test`.

`./rile_din_lib_run test sieve_din` 
...
`gcc -shared -o libsieve_din.so sievelib.o`
...
Содержимое каталога в итоге должно содержать:
```
libsieve_din.so // динамическая библиотека
sievelib.h      // заголовочный файл библиотеки
source/         // папка с исходным кодом функций библиотеки (для работы не нужна, можно и удалить)
```
...
Компилируем его с библиотекой:
`gcc -o push_test push_test.o -L.. -lsieve_din -Wl,-rpath,..`
 
* `-o push_test`- компилируется исполняемый файл `push_test`;
* `push_test.o` - из объектного файла;
* `-L..`        - после **-L** пишем адрес каталога, где находится библиотека (**..** - вышестоящий каталог);
* `-lsieve_din` - после **-l** пишется имя библиотеки без префикса **lib** и суффикса **.so**;
